 ...
 */
export async function getFromCache<T>(key: string, config: CacheConfig): Promise<T | null> {
	try {
 ...
 */
export async function setInCache<T>(key: string, data: T, config: CacheConfig): Promise<void> {
	try {
		const cacheName = config.cacheName || "duyet-mcp";
		const cache = await caches.open(cacheName);
		const cacheKey = generateCacheKey(key, config);
 ...
 */
export async function deleteFromCache(key: string, config: CacheConfig): Promise<void> {
	try {
		const cacheName = config.cacheName || "duyet-mcp";
		const cache = await caches.open(cacheName);
		const cacheKey = generateCacheKey(key, config);
