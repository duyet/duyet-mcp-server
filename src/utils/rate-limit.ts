...
export async function checkRateLimit(
  ...
  const windowStart = Math.floor(
    (now - RATE_LIMIT_CONFIG.TIME_WINDOW_MS) / 1000,
  );
  ...
  try {
    ...
    if (count >= RATE_LIMIT_CONFIG.MAX_PER_PURPOSE_PER_HOUR) {
      return {
        allowed: false,
        reason:
          "Service temporarily unavailable due to high volume. Please try again in a few minutes.",
        retryAfter: 300, // 5 minutes
      };
    }
  } catch (error) {
    // If rate limit check fails, allow the submission (fail open)
    logger.error("auth", "Purpose-based rate limit check failed", {
      purpose,
      error: error instanceof Error ? error.message : String(error),
    });
  }
