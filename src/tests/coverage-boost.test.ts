...
const createMockServer = () => ({ tool: jest.fn() });
...
describe("Coverage Boost Tests", () => {
	describe("Contact Analytics - All Branch Coverage", () => {
		test("should handle summary report with data", async () => {
			...
			mockDb.execute
				.mockResolvedValueOnce([{ purpose: "collaboration", count: 5 }, { purpose: "job_opportunity", count: 3 }])
	...
	describe("Get Contacts - All Path Coverage", () => {
		...
		test("should handle pagination with limit and offset", async () => {
			...
			mockDb.execute.mockResolvedValueOnce(Array.from({ length: 5 }, (_, i) => ({
				...
				json: async () => [
					{
						...
						payload: { commits: [{ message: "Add new feature" }, { message: "Fix bug" }] }
					},
					{
						type: "IssuesEvent",
