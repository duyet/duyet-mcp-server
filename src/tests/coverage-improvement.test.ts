...
const createMockResponse = (data: unknown, ok = true, status = 200) =>
	({
		...
		text: mock(() => Promise.resolve(typeof data === "string" ? data : JSON.stringify(data))),
...
describe("Coverage Improvement Tests", () => {
	...
	describe("GitHub Activity - Tool Implementation", () => {
		test("should register and execute GitHub activity resource", async () => {
		...
		test("should handle GitHub tool API errors", async () => {
			...
			mockFetch.mockResolvedValue(createMockResponse(null, false, 404) as Response);
